...
interface
uses Windows, WinSock, AppStructs,
{$IFDEF USE_CLASSES}
  Classes
{$ELSE}
  uThread, uList
...
type
  ...
  public
    ...
    property NumSocket:TSocket read FSocket;
  ...
  private
    ...
    FWaitingTransaction:Boolean;
    FCodTransWaiting:String[10];
    FPSocketData:PSocketData;
  protected
    procedure Execute; override;
  public
    constructor Create(Connection: TTCPConnection);
    function WaitTransaction(CodTrans:String ; var sktData:TSocketData; Timeout:Integer=20000):Boolean;
    procedure SetResponse(SktData:TSocketData);
...
begin
  ...
  FPSocketData := nil;
  FWaitingTransaction := False;
  FCodTransWaiting    := '9999999999';
  Resume;
end;
...
procedure TTCPConnectionThread.SetResponse(SktData: TSocketData);
begin
    If Assigned(FPSocketData) Then
    begin
       Dispose(FPSocketData);
       FPSocketData := nil;
    end;
    FPSocketData                := New(PSocketData);
    FPSocketData.Command        := SktData.Command;
    FPSocketData.CodTransaction := sktData.CodTransaction;
    FPSocketData.Value          := SktData.Value;
    FPSocketData.GameState      := SktData.GameState;
    FPSocketData.Player.Index   := SktData.Player.Index;
    FPSocketData.Player.Name    := SktData.Player.Name;
    FPSocketData.Player.IP      := SktData.Player.IP;
    FPSocketData.Player.Team    := SktData.Player.Team;
    FPSocketData.Player.Client  := SktData.Player.Client;
    FPSocketData.ErrorMsg       := SktData.ErrorMsg;
    FPSocketData.Team           := SktData.Team;
    FPSocketData.NotifyClient   := SktData.NotifyClient;
end;

function TTCPConnectionThread.WaitTransaction(CodTrans: String; var sktData: TSocketData; Timeout: Integer): Boolean;
var
  ms:Integer;
begin
    Result := False;
    If not FWaitingTransaction then
       FWaitingTransaction := True;

    if CodTrans <> FCodTransWaiting Then
       FCodTransWaiting := CodTrans;

    ms := GetTickCount;
    Repeat
       Sleep(100);
    Until ((FPSocketData <> NIL) And (FPSocketData.CodTransaction = FCodTransWaiting)) or
          ((GetTickCount-ms) > Timeout);

    If (FPSocketData <> Nil) Then
    Begin
       Result := True;
       sktData.Command        := FPSocketData.Command;
       sktData.CodTransaction := FPSocketData.CodTransaction;
       sktData.Value          := FPSocketData.Value;
       sktData.GameState      := FPSocketData.GameState;
       sktData.ErrorMsg       := FPSocketData.ErrorMsg;
       sktData.Player         := FPSocketData.Player;
       sktData.Team           := FPSocketData.Team;
       sktData.NotifyClient   := FPSocketData.NotifyClient;  
       Dispose(FPSocketData);
       FPSocketData := nil;
       FWaitingTransaction := False;
       FCodTransWaiting    := '9999999999';
    end else
       raise ENoResponse.Create('Não houve resposta para a transação: '+ CodTrans);
end;
